...
const i18n = {
    en: {
        ...
        introBody: "This simulation calculates the financial performance of a property investment over a specified period. It takes into account purchase price, mortgage rate, rental income, and expenses.",
        ...
        footerDisclaimer: "Results are illustrative only and should not be considered personalized financial or tax advice.",
    ...
    de: {
        ...
        introBody: "Diese Simulation berechnet die finanzielle Performance einer Immobilieninvestition über einen Zeitraum. Sie berücksichtigt Kaufpreis, Hypothekenzins, Mieteinnahmen und Ausgaben.",
        ...
        footerDisclaimer: "Ergebnisse sind illustrativ und ersetzen keine individuelle Finanz- oder Steuerberatung.",
    ...
    zh: {
        ...
        introBody: "该模拟计算在指定周期内房产投资的财务表现，涵盖购房价格、贷款利率、租金收入与费用。",
...
const furnitureTableBody = document.querySelector("#furniture_table tbody");
const addRowButton = document.getElementById("add_furniture_row");
const calculateFurnitureButton = document.getElementById("calculate_furniture");
...
function getLocale() {
    if ((languageSelect === null || languageSelect === void 0 ? void 0 : languageSelect.value) === "de")
        return "de-DE";
    if ((languageSelect === null || languageSelect === void 0 ? void 0 : languageSelect.value) === "zh")
        return "zh-CN";
    return "en-US";
}
function calculateIRR(cashFlows, guess = 0.1, iterations = 100, tolerance = 1e-6) {
    let rate = guess;
...
function estimateIncomeTax(grossIncome) {
...
function calculateFurnitureDepreciation({ method = "berlin", items = {}, year = 1, hamburg_interest_rate = 0.12, hamburg_depreciation_rate = 0.15, hamburg_depreciation_years = 7, }) {
    ...
    return entries.reduce((total, [value, lifespan]) => {
        ...
        const depreciatingValue = effectiveYear > hamburg_depreciation_years
            ? value * 0.3
...
function buildFurnitureItems(items, lifespans) {
    var _a, _b, _c;
    const lifespanMap = {
        kitchen: Math.max(1, Number((_a = lifespans.kitchen) !== null && _a !== void 0 ? _a : 10)),
        appliances: Math.max(1, Number((_b = lifespans.appliances) !== null && _b !== void 0 ? _b : 5)),
        furniture: Math.max(1, Number((_c = lifespans.furniture) !== null && _c !== void 0 ? _c : 10)),
    };
    return items.reduce((acc, item) => {
        ...
        if (name && Number.isFinite(value)) {
            const lifespan = (_a = lifespanMap[category]) !== null && _a !== void 0 ? _a : 1;
            acc[name] = [Number(value), Number(lifespan)];
        }
        return acc;
    }, {});
}
function calculateLoanTermForMonthlyPayment(loanAmount, annualInterestRate, maxYears = 30) {
...
function propertyInvestmentCalculator({ purchase_price, mortgage_rate, loan_percentage, rental_income_monthly, hausgeld_monthly, grundsteuer_yearly, maintenance_reserve_per_sqm_yearly, apartment_size_sqm, principal_repayment_rate = 0.02, salary_income = 60000, monthly_expenses = 2000, salary_increase_rate = 0.02, years = 10, vacancy_rate = 0.05, property_transfer_tax_rate = 0.06, provision_rate = 0.0357, notary_fee_rate = 0.015, grundbuch_fee_rate = 0.005, renovation_costs = 0, depreciation_rate = 0.02, land_value_per_sqm = 0, rental_increase_rate = 0.02, savings_interest_rate = 0.02, furniture_items = null, furniture_depreciation_method = "berlin", inflation_rates = null, appreciation_rates = null, }) {
    ...
    const total_initial_investment = purchase_price + total_purchase_costs + total_furniture_costs + renovation_costs;
    ...
    const building_value = purchase_price + renovation_costs - land_value;
    ...
    const total_expenses_over_years_with = [];
    const total_expenses_over_years_without = [];
    const tax_over_years_with = [];
    const tax_over_years_without = [];
    const rental_incomes = [];
    const operating_cashflows = [];
    ...
    const net_wealth_over_years_with = [];
    const net_wealth_over_years_without = [];
    const years_list = [];
    const initial_investment_with_furniture = purchase_price * (1 - loan_percentage) +
        purchase_price *
            (property_transfer_tax_rate + provision_rate + notary_fee_rate + grundbuch_fee_rate) +
        renovation_costs +
        total_furniture_costs;
    const cash_flows = [-initial_investment_with_furniture];
    ...
    for (let year = 1; year <= totalYears; year += 1) {
        ...
        const inflation = Array.isArray(inflation_rates) && inflation_rates.length >= year
            ? inflation_rates[year - 1]
        ...
        const inflated_maintenance_reserve = maintenance_reserve_yearly * (1 + random_inflation_rate) ** (year - 1);
        const operating_cashflow = annual_rental_income - inflated_hausgeld - inflated_grundsteuer - inflated_maintenance_reserve;
        operating_cashflows.push(operating_cashflow);
        const salary_now = salary_income * (1 + salary_increase_rate) ** year * (1 + random_inflation_rate);
        const furniture_depr = furniture_items
            ? calculateFurnitureDepreciation({
                method: furniture_depreciation_method,
                items: furniture_items,
                year,
            })
            : 0;
        total_depreciation = depreciation_per_year + furniture_depr;
        const taxable_rental_income = annual_rental_income -
            interest_paid -
            total_depreciation -
            yearly_hausgeld -
            grundsteuer_yearly -
        ...
        const net_cash = operating_cashflows[operating_cashflows.length - 1] - yearly_loan_payment + tax_rental_income;
        ...
        let investment_expenses_yearly = yearly_loan_payment + yearly_hausgeld + grundsteuer_yearly + maintenance_reserve_yearly;
        if (year === 1) {
        ...
        const appreciation = Array.isArray(appreciation_rates) && appreciation_rates.length >= year
    ...
    const total_expenses = total_interest_paid +
        ...
        renovation_costs;
    const latest_balance = (_a = loan_balances[loan_balances.length - 1]) !== null && _a !== void 0 ? _a : balance;
    const latest_property_value = (_b = property_values[property_values.length - 1]) !== null && _b !== void 0 ? _b : property_value;
    const net_profit = latest_property_value - latest_balance + total_rental_income - total_expenses;
    const initial_investment = purchase_price * (1 - loan_percentage) +
        purchase_price *
            (property_transfer_tax_rate + provision_rate + notary_fee_rate + grundbuch_fee_rate) +
        renovation_costs;
    ...
    return {
        ...
        summary: {
            ...
            "Total Expenses with Property": (_c = total_expenses_over_years_with.slice(-1)[0]) !== null && _c !== void 0 ? _c : 0,
            "Total Expenses Without Property": (_d = total_expenses_over_years_without.slice(-1)[0]) !== null && _d !== void 0 ? _d : 0,
            "Loan Balance": latest_balance,
            "Property Value": latest_property_value,
            "Net Wealth With Property": (_e = net_wealth_over_years_with.slice(-1)[0]) !== null && _e !== void 0 ? _e : 0,
            "Net Wealth Without Property": (_f = net_wealth_over_years_without.slice(-1)[0]) !== null && _f !== void 0 ? _f : 0,
            "Operating Cashflow (Last Year)": (_g = operating_cashflows.slice(-1)[0]) !== null && _g !== void 0 ? _g : 0,
            "Tax Paid With Property (Last Year)": (_h = tax_over_years_with.slice(-1)[0]) !== null && _h !== void 0 ? _h : 0,
            "Tax Paid Without Property (Last Year)": (_j = tax_over_years_without.slice(-1)[0]) !== null && _j !== void 0 ? _j : 0,
...
function updateRangeValue(input) {
    const display = document.querySelector(`[data-range-value-for="${input.id}"]`);
    if (display) {
        display.textContent = `${input.value}%`;
    }
}
function createFurnitureRow(item = { name: "", category: "kitchen", value: 0 }) {
    ...
    <option value="kitchen">${i18n[languageSelect.value].categoryKitchen}</option>
    <option value="appliances">${i18n[languageSelect.value].categoryAppliances}</option>
    <option value="furniture">${i18n[languageSelect.value].categoryFurniture}</option>
  `;
...
function updateFurnitureCategoryLabels(lang) {
...
function collectFurnitureItems() {
    ...
    return Array.from(rows).map((row) => {
        const [nameInput, categorySelect, valueInput] = row.querySelectorAll("input, select");
        return {
...
function getFurnishingOption() {
    const selected = document.querySelector("input[name='furnishing_option']:checked");
    return selected ? selected.value : "unfurnished";
}
function updateSummaryLabels(lang) {
    const list = document.getElementById("result_stats");
    if (!list)
        return;
    list.querySelectorAll("li").forEach((item) => {
        const labelKey = item.getAttribute("data-label-key");
        if (labelKey) {
...
function formatValue(value, format, locale = getLocale()) {
...
function renderSummary(summary) {
    ...
    const list = document.getElementById("result_stats");
    list.innerHTML = "";
    const wealthDelta = ((_a = summary["Net Wealth With Property"]) !== null && _a !== void 0 ? _a : 0) - ((_b = summary["Net Wealth Without Property"]) !== null && _b !== void 0 ? _b : 0);
    ...
    summaryConfig.forEach((config) => {
        ...
        label.textContent = i18n[languageSelect.value][config.label];
...
function calculateFurniture() {
    ...
    const depreciation = calculateFurnitureDepreciation({
        method: getFormValue("depreciation_method"),
    ...
    if (costNode)
        costNode.textContent = formatValue(totalCost, "currency");
    if (depNode)
        depNode.textContent = formatValue(depreciation, "currency");
}
let lastFigure = null;
...
function runSimulation() {
    ...
    const furnitureItems = furnishingOption === "furnished" ? buildFurnitureItems(collectFurnitureItems(), lifespans) : null;
    const result = propertyInvestmentCalculator({
        ...
        furniture_depreciation_method: getFormValue("depreciation_method"),
    });
...
function init() {
    ...
    addRowButton === null || addRowButton === void 0 ? void 0 : addRowButton.addEventListener("click", () => createFurnitureRow());
    calculateFurnitureButton === null || calculateFurnitureButton === void 0 ? void 0 : calculateFurnitureButton.addEventListener("click", calculateFurniture);
    runSimulationButton === null || runSimulationButton === void 0 ? void 0 : runSimulationButton.addEventListener("click", runSimulation);
}
init();
