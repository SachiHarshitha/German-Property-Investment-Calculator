...
function updateSummaryLabels(lang) {
  ...
  list.querySelectorAll("li").forEach((item) => {
    ...
    if (labelKey) {
      item.querySelector("span").textContent = i18n[lang][labelKey];
    }
  });
}
...
function renderSummary(summary) {
  const list = document.getElementById("result_stats");
  list.innerHTML = "";
  const locale = languageSelect.value === "de" ? "de-DE" : languageSelect.value === "zh" ? "zh-CN" : "en-US";

  const wealthDelta = summary["Net Wealth With Property"] - summary["Net Wealth Without Property"];
  ...
  summaryConfig.forEach((config) => {
    ...
    value.textContent = formatValue(mergedSummary[config.key], config.format, locale);
...
async function calculateFurniture() {
  const payload = {
    ...
    items: collectFurnitureItems(),
  };

  const response = await fetch("/api/furniture-depreciation", {
    method: "POST",
  ...
  document.getElementById("furniture_cost").textContent = new Intl.NumberFormat(locale, {
    style: "currency",
    currency: "EUR",
  }).format(data.total_cost);
  document.getElementById("furniture_depreciation").textContent = new Intl.NumberFormat(locale, {
    style: "currency",
    currency: "EUR",
...
async function runSimulation() {
  const payload = {
    ...
    salary_increase_rate: Number(getFormValue("salary_increase_rate")),
    savings_interest_rate: Number(getFormValue("savings_interest_rate")),
    ...
    vacancy_rate: Number(getFormValue("vacancy_rate")),
    rental_increase_rate: Number(getFormValue("rental_increase_rate")),
    property_transfer_tax_rate: Number(getFormValue("property_transfer_tax_rate")),
    provision_rate: Number(getFormValue("provision_rate")),
    notary_fee_rate: Number(getFormValue("notary_fee_rate")),
    grundbuch_fee_rate: Number(getFormValue("grundbuch_fee_rate")),
    mortgage_rate: Number(getFormValue("mortgage_rate")),
    loan_percentage: Number(getFormValue("loan_percentage")),
    principal_repayment_rate: Number(getFormValue("principal_repayment_rate")),
    years: Number(getFormValue("years")),
    depreciation_rate: Number(getFormValue("depreciation_rate")),
    furnishing_option: getFurnishingOption(),
    furniture_table: collectFurnitureItems(),
    depreciation_method: getFormValue("depreciation_method"),
    lifespan_kitchen: Number(getFormValue("lifespan_kitchen")),
    lifespan_appliances: Number(getFormValue("lifespan_appliances")),
    lifespan_furniture: Number(getFormValue("lifespan_furniture")),
  };
